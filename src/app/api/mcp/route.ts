...
function normalizeArgs(args: any): any {
  ...
  for (const [key, value] of Object.entries(args)) {
    ...
    // Keep career transition fields as-is (camelCase)
    // currentJob, experience, location, education, mode
  ...
  // Career transition defaults
  if (!normalized.location && normalized.currentJob) normalized.location = 'Australia';
  if (!normalized.mode && normalized.currentJob) normalized.mode = 'report';
...
Ideal for: Users seeking comprehensive job coverage beyond single platforms.`,
    ...
    // Tool 4: Career Transition Advice (AI-Powered Career Intelligence)
    {
      name: 'career_transition_advice',
      description: `[CAREER INTELLIGENCE] Provide personalized career transition recommendations powered by AI analysis of 110,000+ real job market data points from LinkedIn and SEEK. Analyzes career paths, skill gaps, and transition feasibility based on current role and experience level. 

Key features:
• Real job market data from Australia (51K+ unique positions)
• Career graph analysis showing possible transitions
• Skills gap identification (transferable vs. to-develop)
• Salary insights and market demand analysis
• GPT-4.1-mini generated detailed career roadmap
• Phased transition plans (short/mid/long term)
• Industry and role compatibility scoring

Returns: Comprehensive career transition report including:
- Top career transition candidates with similarity scores
- Transferable skills and skills to learn
- Salary expectations and market trends
- Step-by-step action plan with timeline
- Education/certification requirements

Ideal for: Career changers, recent graduates, professionals seeking advancement, international workers in Australia.`,
      input_schema: {
        type: 'object',
        properties: {
          currentJob: {
            type: 'string',
            description: "User's current job title or role (e.g., 'Software Engineer', 'Product Manager', 'Customer Service Representative', 'Accountant'). Can be general or specific.",
          },
          experience: {
            type: 'integer',
            description: "Years of experience in current role (e.g., 2, 5, 10). Use 0 for fresh graduates.",
            minimum: 0,
            maximum: 50,
          },
          location: {
            type: 'string',
            description: "Target location for career transition (e.g., 'Melbourne', 'Sydney', 'Australia'). Optional, defaults to 'Australia'.",
            default: 'Australia',
          },
          education: {
            type: 'string',
            description: "Education level (e.g., 'Bachelor', 'Master', 'High School'). Optional, used for personalization.",
          },
          mode: {
            type: 'string',
            enum: ['raw', 'report'],
            description: "Output mode: 'raw' returns JSON data with career candidates; 'report' returns a comprehensive GPT-generated career transition report (recommended). Default: 'report'",
            default: 'report',
          },
        },
        required: ['currentJob', 'experience'],
        additionalProperties: false,
      },
    },
  ];
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body?.method === "tools/call") {
      ...
      // Validate parameters only for job search tools (not career_transition_advice or get_user_applications)
      if (toolName === 'search_jobs' || toolName === 'build_search_links') {
      ...
      try {
        ...
        switch (toolName) {
          ...
          case 'career_transition_advice':
            result = await Promise.race([
              handleCareerTransitionAdvice(normalizedArgs),
              timeoutPromise
            ]);
            break;
...
// ============================================
// Handler: Career Transition Advice
// ============================================
async function handleCareerTransitionAdvice(args: any) {
  const { currentJob, experience, location = 'Australia', education, mode = 'report' } = args;

  if (!currentJob || typeof currentJob !== 'string') {
    throw new Error('currentJob is required and must be a string');
  }

  if (typeof experience !== 'number' || experience < 0) {
    throw new Error('experience is required and must be a non-negative number');
  }

  // Get Career Switch API URL from environment
  ...
  console.log(`[MCP career_transition_advice] Calling Career Switch API:`, {
    currentJob,
    experience,
    location,
    mode,
  ...
  if (!response.ok) {
    ...
    console.error('[MCP career_transition_advice] API error:', response.status, errorText);
    throw new Error(`Career Switch API error: ${response.status}`);
  }
  ...
  console.log(`[MCP career_transition_advice] Success:`, {
    from: result.from,
